//
//  jscs_command_subclasses.hpp
//  audio engine
//
//  Created by James McCartney on 7/16/25.
//

#ifndef jscs_command_subclasses_h
#define jscs_command_subclasses_h
...
struct NoteSetParamsCmd : Command {
    i64 nodeID_;
    int noteID_;
    std::vector<jscs_ParamPair> values_;

    NoteSetParamsCmd(i64 nodeID, int noteID, int length, jscs_ParamPair* values)
        : nodeID_(nodeID), noteID_(noteID), values_(length)
    {
        memcpy(&values_[0], values, length*sizeof(jscs_ParamPair));
    }
...
#endif /* jscs_command_subclasses_h */
