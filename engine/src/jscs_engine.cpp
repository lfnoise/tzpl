...
void plugInAudioNoOp(jscs_SynthData*) {}

struct OutputNode : jscs_SynthData {
    f32* data;
};

jscs_SynthData* OutputNode_alloc() {
    return (jscs_SynthData*)new OutputNode();
}

jscs_SErr OutputNode_free(jscs_SynthData* synth) {
    delete (OutputNode*)synth;
    return jscs_errNone;
}

jscs_SErr OutputNode_init(jscs_SynthData* synth) {
...
jscs_SErr OutputNode_uninit(jscs_SynthData* synth) {
    OutputNode* o = (OutputNode*)synth;
    delete [] o->data;
    return jscs_errNone;
}
...
void Engine::defOutputNode(int numChannels) {
    ...
    PortInfo inputPortInfo{"in", {jscs_kF32, jscs_audioRate, numChannels}};
...
void Engine::defXFaderNode() {
    ...
    PortInfo a{"a", {jscs_kF32, jscs_audioRate, 0}}; // actual number of channels may vary
    PortInfo b{"b", {jscs_kF32, jscs_audioRate, 0}}; // actual number of channels may vary
    nodeDefInfo.ins = (PortInfo*)calloc(nodeDefInfo.num_ins, sizeof(PortInfo));
    nodeDefInfo.ins[0] = a;
    nodeDefInfo.ins[0] = b;

    PortInfo out{"out", {jscs_kF32, jscs_audioRate, 0}};
