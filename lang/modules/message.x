-- message.x -- a compact binary message format for SExpr values.
--
-- Losslessly encodes/decodes the SExpr enum (sexprs.x) to/from a `Bytes`
-- buffer, and offers a zero-copy Reader for direct field access without
-- materializing an SExpr tree. The same buffer is the carrier for inter-thread
...
-- Node kinds; discriminants line up with the wire tags and SExpr cases.
enum MsgTag {
    bool,
    int,
    float,
...
fn _encodeValue(buf Bytes, o SExpr) Enc {
    match (o) {
        SExpr.bool(b):   Enc{0, false, b ? 1 : 0, 0.0};
        SExpr.int(i):    Enc{1, false, i, 0.0};
        SExpr.float(f):  Enc{2, true, 0, f};
        SExpr.symbol(s): Enc{3, false, _putBlob(buf, s toString), 0.0};
        SExpr.string(s): Enc{4, false, _putBlob(buf, s), 0.0};
        SExpr.vec(v):    Enc{5, false, _putNode(buf, v), 0.0};
    }
}
...
fn _putNode(buf Bytes, v [SExpr]) Int {
    let n = v length;
    var encs [Enc] = [];
    var i = 0;
...
-- Encode an SExpr to a fresh Bytes message.
fn encode(o SExpr) Bytes {
    var buf = bytes();
...
-- =====================================================================
-- Zero-copy reader -- direct access without building an SExpr tree.
...
-- =====================================================================
-- Decoding -- materialize a full SExpr tree (built on the reader).
--
...
-- depth-capped, so a truncated or corrupt buffer yields a small/empty SExpr
...
fn _decodeAt(r Reader, depth Int, budget Ref<Int>) SExpr {
    if (depth > 256 || deref(budget) <= 0) { return SExpr.bool(false); }
    setref(deref(budget) - 1, budget);
    match (tag(r)) {
        MsgTag.bool:   SExpr.bool(asBool(r));
        MsgTag.int:    SExpr.int(asInt(r));
        MsgTag.float:  SExpr.float(asFloat(r));
        MsgTag.symbol: SExpr.symbol(toSymbol(asStr(r)));
        MsgTag.string: SExpr.string(asStr(r));
        MsgTag.vec:    SExpr.vec(_decodeChildren(r, depth, budget));
    }
}

fn _decodeChildren(r Reader, depth Int, budget Ref<Int>) [SExpr] {
    let n = _clampCount(r);
    var out [SExpr] = [];
    var i = 0;
...
-- Decode a TZB message back into an SExpr. Returns an empty vec if `b` is not a
-- valid message (callers handling untrusted input should isMessage-check first).
fn decode(b Bytes) SExpr {
    if (isMessage(b)) {
