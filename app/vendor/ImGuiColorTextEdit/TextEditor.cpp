...
static bool IsWordBoundary(char prev, char cur)
{
	...
	if (isalnum(prev) != isalnum(cur)) return true;
	return false;
}
...
void TextEditor::AddUndo(UndoRecord& aValue)
{
	...
	// Try to merge consecutive single-character insertions or deletions
	// into a single undo record for natural chunked undo behavior.
	if (mUndoIndex > 0)
	{
		auto& prev = mUndoBuffer[mUndoIndex - 1];

		// Merge consecutive character insertions:
		// both records are pure additions (no removal component),
		// both are single characters, not newlines,
		// and the new addition starts where the previous one ended.
		if (aValue.mAdded.size() == 1 && aValue.mRemoved.empty()
			&& !prev.mAdded.empty() && prev.mRemoved.empty()
			&& aValue.mAdded[0] != '\n' && prev.mAdded.back() != '\n'
			&& aValue.mAddedStart == prev.mAddedEnd
			&& !IsWordBoundary(prev.mAdded.back(), aValue.mAdded[0]))
		{
			prev.mAdded += aValue.mAdded;
			prev.mAddedEnd = aValue.mAddedEnd;
			prev.mAfter = aValue.mAfter;
			return;
		}

		// Merge consecutive backspace deletions:
		// both records are pure removals (no addition component),
		// both are single characters, not newlines,
		// and the new removal ends where the previous one started.
		if (aValue.mRemoved.size() == 1 && aValue.mAdded.empty()
			&& !prev.mRemoved.empty() && prev.mAdded.empty()
			&& aValue.mRemoved[0] != '\n' && prev.mRemoved.front() != '\n'
			&& aValue.mRemovedEnd == prev.mRemovedStart
			&& !IsWordBoundary(aValue.mRemoved[0], prev.mRemoved.front()))
		{
			prev.mRemoved = aValue.mRemoved + prev.mRemoved;
			prev.mRemovedStart = aValue.mRemovedStart;
			prev.mBefore = aValue.mBefore;
			prev.mAfter = aValue.mAfter;
			return;
		}
	}
...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			// Draw search highlights for this line
			for (auto& sh : mSearchHighlights)
			{
				if (sh.start.mLine > lineNo || sh.end.mLine < lineNo)
					continue;
				float hstart = (sh.start.mLine == lineNo)
					? TextDistanceToLineStart(sh.start > lineStartCoord ? sh.start : lineStartCoord)
					: 0.0f;
				float hend = (sh.end.mLine == lineNo)
					? TextDistanceToLineStart(sh.end < lineEndCoord ? sh.end : lineEndCoord)
					: TextDistanceToLineStart(lineEndCoord) + mCharAdvance.x;
				if (hstart < hend)
				{
					ImVec2 vstart(lineStartScreenPos.x + mTextStart + hstart, lineStartScreenPos.y);
					ImVec2 vend(lineStartScreenPos.x + mTextStart + hend, lineStartScreenPos.y + mCharAdvance.y);
					auto colorIdx = sh.isCurrent ? PaletteIndex::SearchMatchCurrent : PaletteIndex::SearchMatch;
					drawList->AddRectFilled(vstart, vend, mPalette[(int)colorIdx]);
				}
			}
...
void TextEditor::Backspace()
{
	...
	else
	{
		...
		else
		{
			...
			// Soft tab: if preceded by spaces, delete back to previous tab stop
			int col = mState.mCursorPosition.mColumn;
			int targetCol = ((col - 1) / mTabSize) * mTabSize;
			int spacesToDelete = col - targetCol;
			bool softTab = (spacesToDelete > 1);
			if (softTab)
			{
				for (int i = 0; i < spacesToDelete; ++i)
				{
					int ci = cindex - i;
					if (ci < 0 || line[ci].mChar != ' ')
					{
						softTab = false;
						break;
					}
			...
			if (softTab)
			{
				u.mRemovedStart = Coordinates(pos.mLine, targetCol);
				u.mRemovedEnd = GetActualCursorCoordinates();
				int eraseStart = cindex - spacesToDelete + 1;
				for (int i = 0; i < spacesToDelete; ++i)
				{
...
const TextEditor::Palette & TextEditor::GetDarkPalette()
{
	const static Palette p = { {
			...
			0x4000ffff, // Search match
			0x8000ccff, // Search match (current)
		} };
	return p;
}
...
const TextEditor::Palette & TextEditor::GetLightPalette()
{
	const static Palette p = { {
			...
			0x4000a0ff, // Search match
			0x800080ff, // Search match (current)
		} };
	return p;
}
