...
Both front-ends produce the same internal representation: a directed acyclic graph (DAG) of `Expr` nodes within a `Synth` object. The compiler then runs a series of analysis and optimization passes, generates C++ source code, invokes `clang` to compile it, and links the result into a `.dylib` that conforms to the `jscs_plugin_abi` interface.
...
| File | Purpose |
|------|---------|
| `synthdef_cpp_codegen.hpp/.cpp` | `CppCodeGen` class and associated visitors; emits a complete C++ source file implementing the `jscs_plugin_abi` interface |
...
| `synthdef_plugin_interface.hpp/.cpp` | References the shared `jscs_plugin_abi.h` header (currently a stub) |
...
typedef struct synth_name {
    jscs_SynthFuns funs;          // function pointer table
    struct jscs_Engine* engine;    // audio engine reference
    struct jscs_Node* node;        // node reference
...
| `load()` | Exported C function that returns a `jscs_SynthDef` describing the plugin |
...
Generated plugins conform to the `jscs_plugin_abi.h` interface (located in a sibling `shared/` directory). The key structures:

```c
typedef struct jscs_SynthData {
    jscs_SynthFuns funs;
    struct jscs_Engine* engine;
    struct jscs_Node* node;
    int num_ins, num_outs, num_controls;
    void** inlets;
    void** outlets;
    void** controls;
...
typedef struct jscs_SynthFuns {
    ...
    jscs_SErr (*free)(jscs_SynthData*);
    jscs_SErr (*init)(jscs_SynthData*);
    jscs_SErr (*uninit)(jscs_SynthData*);
    jscs_SErr (*reset)(jscs_SynthData*);
    jscs_SErr (*event)(jscs_SynthData*, u64, jscs_Slice, jscs_Slice);
    void (*processEvents)(jscs_SynthData*);
    void (*processAudio)(jscs_SynthData*);
...
The generated `.dylib` exports a single `load()` function that returns a `jscs_SynthDef`. The audio engine calls `dlopen()` to load the plugin, then uses the function pointer table to manage the synth's lifecycle.
...
The build directory defaults to `~/sapf-build-5/` or the path in the `SAPF3_BUILD` environment variable.

### Runtime Audio

For testing, the compiler can immediately play the generated synth using RtAudio (Core Audio on macOS):
1. `loadDef()` -- `dlopen()` the `.dylib`, call `load()` to get the `jscs_SynthDef`
