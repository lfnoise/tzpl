...
-- (runActorServer) drains it, decodes the Msg, and delivers it by name to the
...
async fn conductor(self Actor<Msg>, init Msg) Void {
    while (true) {
        let m = await receive(self);
        match (m) {
            Msg.float(p): { "NRT GOT " print; p println; }
            _: {}
        }
    }
}
...
conductor spawn(Msg.int(0)) register('conductor);
